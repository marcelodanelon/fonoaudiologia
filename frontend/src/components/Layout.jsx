...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    if (path === '/recepcao' || path === '/atendimentos') setOpenSection('atendimento');
    else if (path === '/pacientes' || path === '/operadores') setOpenSection('cadastros');
        ...
        <div className="modal-overlay" style={{ zIndex: 300 }}>
          <div className="modal" style={{ maxWidth: 420, textAlign: 'center' }}>
            <div className="modal-body" style={{ padding: '36px 32px' }}>
              ...
              <h3 style={{ marginBottom: 8, fontWeight: 700, fontSize: 18 }}>Sessao Expirando</h3>
              <p style={{ marginBottom: 24, color: 'var(--text-muted)', fontSize: 14, lineHeight: 1.6 }}>
                Sua sessao expira em menos de 1 minuto por inatividade.
              </p>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
                <button className="btn btn-primary" onClick={extendSession} style={{ flex: 1 }}>Continuar Logado</button>
          ...
          {hasPermission('consultation') || hasPermission('reception') ? (
            <div>
              <div className="nav-section clickable" onClick={() => toggle('relatorios')}>
                <span>Relatorios</span>{chevron(openSection === 'relatorios')}
              </div>
          ...
          {hasPermission('auditLog') || hasPermission('systemConfig') ? (
            <div>
              <div className="nav-section clickable" onClick={() => toggle('administracao')}>
                <span>Administracao</span>{chevron(openSection === 'administracao')}
              </div>
