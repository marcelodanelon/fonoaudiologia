...
export default function Consultation() {
  ...
  const [form, setForm] = useState({
    patientId: '', professionalId: '', type: 'CONSULTA', status: 'AGENDADA',
  ...
  useEffect(() => { loadData(); loadReadyPatients(); }, [selectedDate]);
  ...
  const loadData = async () => {
    try {
      const [cRes, pRes, profRes] = await Promise.all([
        api.get('/consultations', { params: { startDate: selectedDate, endDate: selectedDate } }),
        api.get('/patients'),
        api.get('/users'),
      ]);
  ...
  const loadReadyPatients = async () => {
    try {
      const [readyRes, aptRes] = await Promise.all([
        api.get('/reception/ready', { params: { startDate: selectedDate, endDate: selectedDate } }),
  ...
  if (view === 'form') {
    const formTabs = [
      ...
      { id: 'exame', label: 'Historico e Exame' },
      { id: 'diagnostico', label: 'Diagnostico e Conduta' },
        ...
        <div className="form-view">
          ...
          {(isLocked || isReadonly) && (
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8, flexShrink: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              {isReadonly ? 'Consulta concluida — somente leitura' : 'Origem da recepcao — campos de identificacao bloqueados'}
            </div>
          )}
          ...
          <div className="form-body">
              ...
              <div className="form-section">
                ...
                <div className="form-group">
                  ...
                  <textarea rows={6} value={form.anamnesis} onChange={e => setForm({...form, anamnesis: e.target.value})} disabled={isReadonly}
                    placeholder="Historico detalhado, queixa, inicio, evolucao..." />
                </div>
              </div>
            )}
            ...
            {formTab === 'exame' && (
              <div className="form-section">
                <div className="form-section-title">Historico Clinico e Exame Fisico</div>
                <div className="form-group">
                  <label>Historico Clinico</label>
                  <textarea rows={5} value={form.clinicalHistory} onChange={e => setForm({...form, clinicalHistory: e.target.value})} disabled={isReadonly}
                    placeholder="Historico medico, cirurgias, medicacoes..." />
                </div>
                <div className="form-group">
            ...
            {formTab === 'diagnostico' && (
              <div className="form-section">
                <div className="form-section-title">Diagnostico e Conduta</div>
                <div className="form-group">
        ...
        <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div className="card-header">
            ...
            {readyPatients.length > 0 && (
              <span className="badge badge-info">{readyPatients.length} na recepcao</span>
            )}
          </div>
          <div className="card-body" style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
            <div className="scroll-container" style={{ flex: 1 }}>
              <table>
                <thead>
                  <tr><th>Data</th><th>Paciente</th><th>Profissional</th><th>Tipo</th><th>Status</th><th>Origem</th><th>Acoes</th></tr>
                </thead>
                <tbody>
                  {filteredConsultations.slice(consultPage * PAGE_SIZE, (consultPage + 1) * PAGE_SIZE).map(c => (
                    <tr key={c.id}>
                      ...
                      <td>{isFromReception(c) ? <span className="badge badge-success">Recepcao</span> : <span className="badge badge-secondary">Direto</span>}</td>
                      <td>
                  ...
                  {filteredConsultations.length === 0 && (
                    <tr><td colSpan={7} className="empty-state" style={{ padding: 40 }}>Nenhuma consulta encontrada</td></tr>
                  )}
                </tbody>
              </table>
            </div>
        ...
        <div className="card" style={{ flexShrink: 0, marginTop: 4 }}>
          <div className="card-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <h3 style={{ margin: 0 }}>Pacientes na Recepcao</h3>
